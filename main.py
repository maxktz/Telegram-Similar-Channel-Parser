...
class SimilarChannelParser:
    ...
    async def get_similar_channels(self, channel_entity: str) -> list[str]:
        ...
        async with self.client:
            try:
                req = functions.channels.GetChannelRecommendationsRequest(
                    channel_entity
                )
                ...
                return []
        channels: list[str] = [
            config.LINE_FORMAT.format(**chat.__dict__) for chat in res.chats
        ]
    ...
    async def main(self):
        ...
        while True:
            channel_username = input(
                "Please input channel username (leave empty to stop): "
            )
